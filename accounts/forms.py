...
class UserLogin(AuthenticationForm):
    ...
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control mb-3', 'placeholder':'Password', 'id': 'login-psw'}))
...
class RegisterForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if r.count():
            raise ValueError('Username aready exists')
        return username
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('You already have account with us. Please try log in.')
        return email
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['password2'].widget.attrs.update({'class':'form-control mb-3', 'placeholder': 'Password'})